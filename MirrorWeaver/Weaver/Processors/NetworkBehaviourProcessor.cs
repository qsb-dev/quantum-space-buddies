...
namespace Mirror.Weaver
{
	...
	internal class NetworkBehaviourProcessor
	{
		...
		private readonly SyncVarAttributeProcessor syncVarAttributeProcessor;
        ...
        */
		...
		public static bool WasProcessed(TypeDefinition td) => td.GetMethod(ProcessedFunctionName) != null;
        ...
        */
		...
		{
			// pass full function name to avoid ClassA.Func <-> ClassB.Func collisions
			var cmdName = cmdResult.method.FullName;
		...
		private void GenerateSerialization(ref bool WeavingFailed)
		{
			...
			var dirtyBit = syncVarAccessLists.GetSyncVarStart(netBehaviourSubclass.BaseType.FullName);
			foreach (var syncVar in syncVars)
			{
		...
		private void DeserializeField(FieldDefinition syncVar, ILProcessor worker, MethodDefinition deserialize, ref bool WeavingFailed)
		{
		...
		private void DeserializeNetworkIdentityField(FieldDefinition syncVar, ILProcessor worker, MethodDefinition deserialize, MethodDefinition hookMethod, ref bool WeavingFailed)
		{
			/*
            Generates code like:
		...
		private void DeserializeNetworkBehaviourField(FieldDefinition syncVar, ILProcessor worker, MethodDefinition deserialize, MethodDefinition hookMethod, ref bool WeavingFailed)
		{
			/*
            Generates code like:
		...
		private void DeserializeNormalField(FieldDefinition syncVar, ILProcessor serWorker, MethodDefinition deserialize, MethodDefinition hookMethod, ref bool WeavingFailed)
		{
			/*
             Generates code like:
                // for hook
